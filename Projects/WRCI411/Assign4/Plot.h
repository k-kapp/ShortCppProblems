...
public:
	...
	Plot(vector<double> basic_hist_vec) : basic_hist_vec(basic_hist_vec)
	{
	}
	...
	boost::tuple<vector<int>, vector<int>, vector<double> > process_basic_hist_data()
	{
		boost::tuple<vector<int>, vector<int>, vector<double> > new_data(vector<int>(basic_hist_vec.size()), vector<int>(basic_hist_vec.size()), vector<double>(basic_hist_vec.size()));
/*
		for (int i = 0; i < basic_hist_vec.size(); i++)
		{
			new_data.first[i] = 5*i + 3;
			new_data.second[i] = basic_hist_vec[i];
		}
*/

		for (int i = 0; i < basic_hist_vec.size(); i++)
		{
			new_data.get<0>()[i] = 5*i + 3;
			new_data.get<1>()[i] = i + 1;
			new_data.get<2>()[i] = basic_hist_vec[i];
		}
		return new_data;
	}
	
	pair<vector<int>, vector<double> > process_basic_line_data()
	{
		pair<vector<int>, vector<double> > new_data(vector<int>(basic_hist_vec.size()), vector<double>(basic_hist_vec.size()));

		for (int i = 0; i < basic_hist_vec.size(); i++)
		{
			new_data.first[i] = i + 1;
			new_data.second[i] = basic_hist_vec[i];
		}
		return new_data;
	}

	void plot_basic_hist(string title)
	{
		Gnuplot gp;

		//pair<vector<int>, vector<double> > dat = process_basic_hist_data();
		boost::tuple<vector<int>, vector<int>, vector<double> > dat = process_basic_hist_data();
		gp << "set boxwidth 0.8\n";
		gp << "set style fill solid\n";
		//gp << "set xtics 1, 10, 81\n";

		gp << "plot '-' using 2:3 with boxes title '" << title << "'\n";
		//gp << "set xtics 1, 10, 81\n";
	...
	void plot_basic_line()
	{
		Gnuplot gp;

		pair<vector<int>, vector<double> > dat = process_basic_line_data();

		gp << "set boxwidth 4\n";
		gp << "set style fill solid\n";

		gp << "plot '-' using 1:2 with lines title 'Historical Fitness Values'\n";
